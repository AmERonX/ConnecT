...
const filepath = "c:\\Projects\\ConnecT\\frontend\\js\\pages\\teams.js";
let content = fs.readFileSync(filepath, 'utf8');

const old_card_start = `      <div class="team-card slide-up">
...
const new_card_layout = `      <div class="team-card slide-up">
        <div class="team-card-header">
              ...
              \${(team.members || []).map(member => \`
                <div class="member-row" style="padding:8px 12px;border:0.5px solid var(--border);border-radius:8px;background:var(--bg-card);justify-content:space-between">
                  ...
                  \${team.completed && member.id !== session.user.id ? (
                    ...
                    \`<button style="background:none;border:none;color:var(--text-red);cursor:pointer;font-size:0.875rem;text-decoration:underline" data-action="kick-member" data-team-id="\${team.id}" data-user-id="\${member.id}">Kick</button>\`
                  ) : '')}
                </div>
              \`).join('')}
            </div>
...
if (content.includes(old_card_start)) {
    content = content.replace(old_card_start, new_card_layout);
    fs.writeFileSync(filepath, content, 'utf8');
    console.log("Successfully replaced layout in teams.js");
} else {
    console.log("Could not find old layout chunk in teams.js");
}
